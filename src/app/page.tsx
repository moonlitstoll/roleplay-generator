...
export default function Home() {
  ...
  // Effect: When entering merged mode, trigger merge if needed
  useEffect(() => {
    if (isMergedMode && !mergedAudioUrl && !isMerging && Object.keys(audioUrls).length > 0) {
      const activeUrls = (language === 'Vietnamese' && vietnameseAccent === 'south') ? audioUrlsSouth : audioUrls;
      mergeAudio(activeUrls);
    }
  }, [isMergedMode, mergedAudioUrl, audioUrls, audioUrlsSouth, vietnameseAccent, language]);
      ...
      <div className="glass px-4 lg:px-6 py-4 rounded-2xl flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white shadow-sm border border-gray-200">
        ...
        <div className="flex items-center justify-between lg:justify-end gap-3 w-full lg:w-auto overflow-x-auto pb-1 lg:pb-0 no-scrollbar">
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <button
              onClick={() => setIsMergedMode(!isMergedMode)}
              className={`p-2.5 rounded-xl border transition-all relative group ${isMergedMode ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
              title="Continuous Audio Mode (Best for Background Play)"
            >
              ...
              <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">Background Mode</span>
            </button>
