  ...
  try {
    ...
    const prompt = `
      ...
      **[8가지 분석 규칙]**
      ...
      5. **최소한의 뼈대 공식 (한국어 문법 용어 전용)**: 구체적인 명사나 수식어는 걷어내고 어디든 갈아 끼울 수 있는 최소한의 공식만 추출한다. **변수명으로 베트남어/영어 용어를 쓰는 것을 엄격히 금지하며, 반드시 아래의 처리 과정을 거쳐 한국어 대응어만 출력한다.**
         - **[내부 처리 과정]**:
            1단계: 문장의 문법 요소 식별 (예: 'Chúng ta'는 주어, 'khám phá'는 동사)
            2단계: 식별된 요소에 대응하는 한국어 용어를 아래 [매핑 가이드]에서 확인
            3단계: **생각은 베트남어로 하더라도, 최종 출력(structure)에는 반드시 매핑된 한국어 용어만 사용**
         - **[용어 매핑 가이드 (필수)]**:
            *   Chủ ngữ / Subject -> **[주어]**
