  ...
  try {
    ...
    const prompt = `
      ...
      **[8가지 분석 규칙]**
      ...
      5. **최소한의 뼈대 공식 (한국어 변수 사용)**: 구체적인 명사나 수식어는 걷어내고 어디든 갈아 끼울 수 있는 최소한의 공식만 추출한다. 이때, **변수명은 반드시 한국어로 작성한다**. 모든 문장에 대해 **반드시** 이 항목을 작성해야 한다.
         - (X) [Danh từ] có [Danh từ]
         - (O) [명사] có [명사]
      ...
      **[출력 데이터 매핑]**
      ...
      - \`patterns\`: 
         - \`structure\`: 최소한의 뼈대 공식 (예: [주어] càng [동사1], [주어] sẽ càng [동사2]) **(반드시 한국어 변수 사용)**
